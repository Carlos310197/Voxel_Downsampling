...
void first_pass(int* surface, int num_triangles, int* voxel_list, int* new_surface)
{
	...
	{
		// printf("surface: %d %d %d\n",surface[i*3+0],surface[i*3+1],surface[i*3+2]);
		...
		// printf("new_surface: %d %d %d\n",new_surface[i*3+0],new_surface[i*3+1],new_surface[i*3+2]);
	}
}
...
int second_pass(int* surface, int num_triangles)
{
	...
	for (int i = 0; i < num_triangles; i++)
	{
		...
		{
			temp_surface[counter * 3 + 0] = s[0];
			temp_surface[counter * 3 + 1] = s[1];
			temp_surface[counter * 3 + 2] = s[2];
			counter++;
		}
	}

	// Copy out the result
	memcpy(surface, temp_surface, (size_t)counter * (size_t)3 * sizeof(int));
	// for(int i=0; i<counter*3;i++) surface[i] = temp_surface[i];
